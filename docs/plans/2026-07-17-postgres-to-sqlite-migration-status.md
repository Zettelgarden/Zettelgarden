...
**Tracking:** epic `Zettelgarden-c7j` · Phase 0 `Zettelgarden-bw1` (closed) · Phase 1 `Zettelgarden-2u2` (closed) · Phase 2 `Zettelgarden-dek` (closed)

## TL;DR

Implementation is underway. **Phases 0, 1, and 2 done.** Every high-risk
*unknown* was resolved empirically in Phase 1 (which shrank the plan: the
1373-edit placeholder sweep is gone, and a latent timestamp landmine was caught
and fixed schema-side). Phase 2 produced the consolidated SQLite schema
(`go-backend/schema/sqlite/schema.sqlite.sql`, 73 tables / 134 indexes)
translated from a live `pg_dump --schema-only` snapshot; it loads cleanly into
`:memory:` via the Phase 1 statement splitter with `PRAGMA foreign_key_check`
and `integrity_check` both green. **Phase 3 (query translation sweep) is next**
— shrunk to ~1 day (placeholders already eliminated by Phase 1).
...
| 3 — Query translation | ⬜ Not started | **Next up.** Shrunk to ~1 day — placeholder sweep eliminated; remaining: `NOW()`/`INTERVAL`/`ILIKE`/casts only |
...
Phase 2 is **done** — the consolidated SQLite schema exists and validates.

1. **Phase 3 (query translation sweep, ~1 day).** Placeholder sweep already
   eliminated by Phase 1 (modernc binds `$1` natively). Remaining non-test work:
   - `NOW()` → app-side `time.Now().UTC()` — ~99 occurrences remaining across
     31 files (Phase 1 did the first 4 sites in `services/cards.go` to prove the
     pattern).
   - `INTERVAL '…'` → `datetime(…, '+N unit')` — 6 files: `models/job.go`,
     `handlers/admin/stats.go`, `services/{jobs/cleanup_job,
     jobs/rss_article_cleanup_job, stats, smart_feed}.go`.
   - `ILIKE` → `LIKE` (verify non-ASCII case-folding) — 5 files:
     `handlers/{search,facts,files}.go`, `services/{cards,facts}.go`.
   - `::` casts — `services/stats.go` (the `server/sqlite.go` hit is already
     SQLite code).
2. **Phase 6b ETL:** no CGO fallback needed — modernc inserts 15k cards in
   ~0.8s (Phase 1 probe). ETL is pure Go: `lib/pq` to read PG, modernc to write
   SQLite.
