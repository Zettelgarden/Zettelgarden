...
| 5 — Triggers → Go | 🔄 **In progress** | user_stats counters + llm_jobs.updated_at **done** (migration 0145 drops the replaced PG triggers; Go maintains on both drivers per decision 3b). rss notifications + chat/email/notify dead-trigger drops **next**. See *Phase 5 progress* below. |
...
**Next:** rss notifications (`0124` + rss part of `0122`) — reuse
`models.CreateNotification` / `CalculateImportanceScore` / `GetFilterTagsForRSS`
at the rss_articles insert/star/read/update/delete sites; add
`models.DeleteNotificationBySource`; migration 0146 drops `0122`/`0123`/`0124`.
