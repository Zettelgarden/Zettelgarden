...
func RegisterEmailRoutes(r *mux.Router, h *handlers.Handler) {
	...
	// IMPORTANT: Register specific routes before wildcard/{id} routes to avoid path conflicts
	// Static routes (no path variables)
	addProtectedRoute(r, h, "/api/emails/search", h.SearchEmailsRoute, "GET")
	...
	// Wildcard routes (with path variables) - must come after static routes
	...
	// Batch operation routes (static routes)
	...
	// Email attachment routes (static and wildcard)
