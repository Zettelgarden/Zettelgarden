// Package article provides article-related data access and business logic
// for the Zettelgarden tool registry.
//
// This package contains functions for parsing URLs and creating article cards.
package article

import (
	"database/sql"
	"fmt"
	"log"
	"time"

	htmltomarkdown "github.com/JohannesKaufmann/html-to-markdown/v2"
	readability "github.com/go-shiori/go-readability"
	"go-backend/models"
)

// ParseResult represents the result of parsing a URL
type ParseResult struct {
	Title    string `json:"title"`
	Content  string `json:"content"`
	Author   string `json:"author,omitempty"`
	Excerpt  string `json:"excerpt,omitempty"`
	SiteName string `json:"site_name,omitempty"`
	URL      string `json:"url"`
}

// ParseURL extracts article content from a URL for preview
func ParseURL(url string) (*ParseResult, error) {
	if url == "" {
		return nil, fmt.Errorf("url is required")
	}

	// Parse the URL using readability
	article, err := readability.FromURL(url, 30*time.Second)
	if err != nil {
		return nil, fmt.Errorf("failed to parse URL: %w", err)
	}

	// Convert HTML content to markdown
	markdown, err := htmltomarkdown.ConvertString(article.Content)
	if err != nil {
		log.Printf("Failed to convert to markdown: %v", err)
		markdown = article.Content // Fallback to HTML
	}

	return &ParseResult{
		Title:    article.Title,
		Content:  markdown,
		Author:   article.Byline,
		Excerpt:  article.Excerpt,
		SiteName: article.SiteName,
		URL:      url,
	}, nil
}

// CreateArticle creates a card from a URL with auto-parsed content
func CreateArticle(db *sql.DB, userID int, url, cardID, tags string) (*models.Card, error) {
	// Parse the URL
	parsed, err := ParseURL(url)
	if err != nil {
		return nil, fmt.Errorf("failed to parse URL: %w", err)
	}

	// Default tags if not provided
	if tags == "" {
		tags = "#to-read #reference"
	}

	// Combine tags with content
	body := fmt.Sprintf("%s\n\n%s", parsed.Content, tags)

	// Create the card - use the local CreateCard function
	params := models.EditCardParams{
		Title:  parsed.Title,
		Body:   body,
		Link:   url,
		CardID: cardID,
	}

	card, err := CreateCard(db, userID, params)
	if err != nil {
		return nil, fmt.Errorf("failed to create card: %w", err)
	}

	return &card, nil
}

// CreateCard is a local copy of the card creation function to avoid circular imports.
// This is a simplified version that handles the basic card creation.
func CreateCard(db *sql.DB, userID int, params models.EditCardParams) (models.Card, error) {
	// Generate a card_id if not provided
	cardID := params.CardID
	if cardID == "" {
		cardID = generateCardID(db, userID)
	}

	// Default title if not provided
	title := params.Title
	if title == "" {
		title = "Untitled"
	}

	// Insert the card
	query := `
		INSERT INTO cards (user_id, card_id, title, body, link, created_at, updated_at)
		VALUES ($1, $2, $3, $4, $5, NOW(), NOW())
		RETURNING id, user_id, card_id, title, body, link, created_at, updated_at
	`

	var card models.Card
	err := db.QueryRow(query, userID, cardID, title, params.Body, params.Link).Scan(
		&card.ID,
		&card.UserID,
		&card.CardID,
		&card.Title,
		&card.Body,
		&card.Link,
		&card.CreatedAt,
		&card.UpdatedAt,
	)

	return card, err
}

// generateCardID generates a unique card_id for a new card
func generateCardID(db *sql.DB, userID int) string {
	// Try to find the next available ID
	for i := 1; i <= 100; i++ {
		cardID := fmt.Sprintf("%d", i)
		var exists bool
		err := db.QueryRow("SELECT EXISTS(SELECT 1 FROM cards WHERE user_id = $1 AND card_id = $2)", userID, cardID).Scan(&exists)
		if err == nil && !exists {
			return cardID
		}
	}
	// Fallback to timestamp-based ID
	return fmt.Sprintf("%d", time.Now().Unix())
}
