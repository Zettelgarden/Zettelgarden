...
//
// Migration Status: task_tools is a high-usage domain with 7 tools.
package services
...
import (
	...
	taskpkg "go-backend/services/tools/task"
	"time"
)
...
func handleGetTasksV2(args map[string]interface{}, ctx *ToolContext) (map[string]interface{}, error) {
	...
	if cardPKFloat, ok := args["card_pk"].(float64); ok {
		...
		tasks, err = taskpkg.GetTasksByCard(ctx.DB, ctx.UserID, cardPK)
	} else {
		tasks, err = taskpkg.GetTasks(ctx.DB, ctx.UserID, includeCompleted, "UTC")
	}
...
func handleCreateTaskV2(args map[string]interface{}, ctx *ToolContext) (map[string]interface{}, error) {
	...
	if scheduledDateStr, ok := getOptionalStringParam(args, "scheduled_date"); ok {
		...
		if perr != nil {
			return nil, fmt.Errorf("invalid scheduled_date format: %w", perr)
		}
	...
	if dueDateStr, ok := getOptionalStringParam(args, "due_date"); ok {
		...
		if perr != nil {
			return nil, fmt.Errorf("invalid due_date format: %w", perr)
		}
	...
	taskID, err := taskpkg.CreateTask(ctx.DB, task)
	if err != nil {
		return nil, fmt.Errorf("failed to create task: %w", err)
	}

	newTask, err := taskpkg.GetTask(ctx.DB, ctx.UserID, taskID)
	if err != nil {
...
func handleUpdateTaskV2(args map[string]interface{}, ctx *ToolContext) (map[string]interface{}, error) {
	...
	currentTask, lerr := taskpkg.GetTask(ctx.DB, ctx.UserID, taskID)
	if lerr != nil {
		return nil, fmt.Errorf("failed to get task: %w", lerr)
	}
	...
	uerr := taskpkg.UpdateTask(ctx.DB, ctx.UserID, taskID, currentTask)
	if uerr != nil {
		return nil, fmt.Errorf("failed to update task: %w", uerr)
	}

	updatedTask, uerr := taskpkg.GetTask(ctx.DB, ctx.UserID, taskID)
	if uerr != nil {
		return nil, fmt.Errorf("failed to retrieve updated task: %w", uerr)
	}
...
func handleGetTaskByIDV2(args map[string]interface{}, ctx *ToolContext) (map[string]interface{}, error) {
	...
	task, lerr := taskpkg.GetTask(ctx.DB, ctx.UserID, taskID)
	if lerr != nil {
		return nil, fmt.Errorf("failed to get task: %w", lerr)
	}
...
func handleCompleteTaskV2(args map[string]interface{}, ctx *ToolContext) (map[string]interface{}, error) {
	...
	uerr := taskpkg.UpdateTask(ctx.DB, ctx.UserID, taskID, currentTask)
	if uerr != nil {
		return nil, fmt.Errorf("failed to complete task: %w", uerr)
	}
...
func handleDeleteTaskV2(args map[string]interface{}, ctx *ToolContext) (map[string]interface{}, error) {
	...
	err = taskpkg.DeleteTask(ctx.DB, ctx.UserID, taskID)
	if err != nil {
...
func handleCompleteAndScheduleTaskV2(args map[string]interface{}, ctx *ToolContext) (map[string]interface{}, error) {
	...
	completeStatus, lerr := taskpkg.GetCompleteTaskStatus(ctx.DB, ctx.UserID)
	if lerr != nil {
		return nil, fmt.Errorf("failed to get complete status: %w", lerr)
	}

	defaultStatus, lerr := taskpkg.GetDefaultTaskStatus(ctx.DB, ctx.UserID)
	if lerr != nil {
		return nil, fmt.Errorf("failed to get default status: %w", lerr)
	}

	newTaskID, lerr := taskpkg.CompleteAndScheduleTask(ctx.DB, ctx.UserID, taskID, days, completeStatus.Name, defaultStatus.Name)
	if lerr != nil {
		return nil, fmt.Errorf("failed to complete and schedule task: %w", lerr)
	}
