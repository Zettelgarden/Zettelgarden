...
//
// Migration Status: card_tools is the FINAL Phase 3 domain. All 8 domains complete!
package services
...
// V2 card tool handlers (uses domain package logic)
...
func handleSearchCardsV2(args map[string]interface{}, ctx *ToolContext) (map[string]interface{}, error) {
	...
	// Execute search based on type - use services package for search
	var cards []map[string]interface{}
...
func handleGetCardByIDV2(args map[string]interface{}, ctx *ToolContext) (map[string]interface{}, error) {
	...
	cardData, err := card.GetCardByID(ctx.DB, ctx.UserID, cardID)
	if err != nil {
		return nil, fmt.Errorf("failed to get card: %v", err)
	}

	return card.StructToMap(cardData), nil
}
...
func handleUpdateCardV2(args map[string]interface{}, ctx *ToolContext) (map[string]interface{}, error) {
	...
	// Get the current card using domain package
	currentCard, err := card.GetCardByID(ctx.DB, ctx.UserID, cardPK)
	if err != nil {
		return nil, fmt.Errorf("failed to get card: %v", err)
	}
...
func handleBrowseCardHierarchyV2(args map[string]interface{}, ctx *ToolContext) (map[string]interface{}, error) {
	...
	for _, cardData := range cards {
		results = append(results, card.StructToMap(cardData))
	}
