...
// GetCardsRoute returns cards with optional schema_id filter
func (s *Handler) GetCardsRoute(w http.ResponseWriter, r *http.Request) {
	userID := r.Context().Value("current_user").(int)

	// Parse query parameters
	schemaIDStr := r.URL.Query().Get("schema_id")
	var schemaID *int
	if schemaIDStr != "" {
		id, err := strconv.Atoi(schemaIDStr)
		if err != nil {
	...
	if schemaID != nil {
		query += fmt.Sprintf(" AND card_schema_id = $%d", argPos)
