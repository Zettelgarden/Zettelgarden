...
export function SearchPage({
  ...
  const [showStarSearchDialog, setShowStarSearchDialog] =
    ...
    const initializeSearch = async () => {
      ...
      if (starredId) {
        try {
          ...
          const starredSearch = starredSearches.find(
            (search) => search.id === parseInt(starredId),
          );
  ...
  const handleConfigChangeWithSearch = (
    config: SearchConfigType,
    resetPage?: boolean,
  ) => {
    if (resetPage) {
