...
export function NotificationInboxPage() {
  ...
  const [togglingTaskComplete, setTogglingTaskComplete] = useState<Set<number>>(
    new Set(),
  );
  const [showCreateTaskWindow, setShowCreateTaskWindow] = useState(false);
  const [notificationForTask, setNotificationForTask] =
    useState<Notification | null>(null);
  const [showConvertDialog, setShowConvertDialog] = useState(false);
  const [articleToConvert, setArticleToConvert] = useState<RSSArticle | null>(
    null,
  );
  ...
  const handleNotificationClick = async (notification: Notification) => {
    ...
    switch (notification.source_type) {
      ...
      default:
        console.warn(
          'Unknown notification source type:',
   ...
   */
  const handleCreateTaskFromNotification = (
    notification: Notification,
    event: React.MouseEvent,
  ) => {
    event.stopPropagation();
  ...
  const handleConverted = (cardId: number) => {
    ...
    if (articleToConvert) {
      setNotifications((prev) =>
        prev.filter((n) => n.source_id !== articleToConvert.id),
      );
      setTotal((prev) => prev - 1);
      setUnreadCount((prev) => Math.max(0, prev - 1));
    }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
          ...
          <>
            {notifications.map((notification) => (
              <NotificationItem
                ...
                onCreateTask={(e) =>
                  handleCreateTaskFromNotification(notification, e)
                }
                onConvertToCard={
                  notification.source_type === 'rss'
                    ? (e) => handleConvertToCard(notification, e)
                    : undefined
                }
                onToggleComplete={
                  notification.source_type === 'task'
                    ? (e) => handleToggleTaskComplete(notification, e)
                    : undefined
                }
      ...
      {showCreateTaskWindow && (
        <CreateTaskWindow
          ...
          currentFilter={
            notificationForTask?.title
              ? `Notification: ${notificationForTask.title}`
              : undefined
          }
        />
      )}
