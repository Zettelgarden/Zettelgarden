...
const features: HelpFeature[] = [
  ...
  },
  {
    name: 'Schema Tables',
    description:
      'Display a table from a schema definition. Field names are case-sensitive.',
    syntax: '{{schema: <ref>}}',
    example:
  ...
  },
  {
    name: 'Card Links',
    description:
      'Link to other cards by their ID, optionally with display text or dynamic title',
    syntax: '[[<CardID>]] or [[<CardID>|display text|]] or [[<CardID>|*|]]',
    example:
      '[[my-note]]\n[[20250201-reading-list|January Reading|]]\n[[42|*|]] (resolves to card title)',
  },
];
...
export function CardBodyHelpPopover() {
  return (
    <Popover
      title="Body syntax help"
      triggerClassName="inline-flex items-center justify-center rounded-full bg-blue-100 text-blue-600 min-w-[44px] min-h-[44px] text-xs font-medium hover:bg-blue-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1"
      button={
        <>
          <span className="sr-only">Show body syntax help</span>?
        </>
      }
      panelClassName="absolute z-[60] left-full ml-2 top-0 w-80 max-w-md p-4"
    >
