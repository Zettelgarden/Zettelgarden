...
describe('preprocessSchemaTables', () => {
  ...
  it('should preserve columns and filters together', () => {
    const input =
      '{{schema:1|columns:title,rating|filter:status=active,rating=gte:4}}';
    const output = preprocessSchemaTables(input);
    expect(output).toBe(
      '&SCHEMATABLE:1|columns:title,rating|filter:status=active,rating=gte:4&',
    );
  });
  ...
  it('should handle multiple schema references in one text', () => {
    const input =
      'Check {{schema:1}} and {{schema:book-review|filter:status=active}}';
    const output = preprocessSchemaTables(input);
    expect(output).toBe(
      'Check &SCHEMATABLE:1& and &SCHEMATABLE:book-review|filter:status=active&',
    );
  });
  ...
  it('should handle global insensitive matching', () => {
    ...
    const output = preprocessSchemaTables(input);
    expect(output).toBe(
      '&SCHEMATABLE:1& and &SCHEMATABLE:2& and &SCHEMATABLE:3&',
    );
  });
});
...
describe('preprocessWikiLinks', () => {
  ...
  it('should handle multiple wiki-links in one body', () => {
    ...
    const output = preprocessWikiLinks(input);
    expect(output).toBe(
      'See [a](#card:a) and [Second](#card:b) and [c](#card:c)',
    );
  });
