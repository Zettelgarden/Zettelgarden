...
export function HeaderSubSection({ text, className }: HeaderProps) {
  return (
    <div>
      <span className={`font-bold text-sm ${className || ""}`}>{text}</span>
      <br />
    </div>
  );
}
...
export function H4({ children, className }: HeadlineProps) {
  return <h4 className={`text-base font-bold ${className || ""}`}>{children}</h4>;
}

export function H5({ children, className }: HeadlineProps) {
  return <h5 className={`text-sm font-bold ${className || ""}`}>{children}</h5>;
}
