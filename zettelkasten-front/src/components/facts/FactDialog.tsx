...
export function FactDialog({ onClose, onFactDeleted }: FactDialogProps) {
  ...
  async function handleConfirmMerge() {
    ...
    const primaryFactId =
      mergeDirection === 'into-current' ? selectedFact.id : factToMerge.id;
    const secondaryFactId =
      mergeDirection === 'into-current' ? factToMerge.id : selectedFact.id;
