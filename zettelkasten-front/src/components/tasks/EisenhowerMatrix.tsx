...
export function EisenhowerMatrix({
  ...
  const displayTasks =
    subtaskMode === 'hidden' ? tasks.filter((t) => !t.parent_task_id) : tasks;
      ...
      <Droppable droppableId={quadrant.toString()}>
        {(dropProvided, snapshot) => (
          <div
            ...
            className={`border-2 p-2 rounded min-h-[150px] transition-colors duration-200 ${
          ...
          >
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-0 mb-2">
              <h3 className="font-bold">
                {quadrantTitles[quadrant]} ({quadrantData[quadrant].length})
              </h3>
                ...
                >
                  {(dragProvided) => (
                    <div
                      ...
                      className={`mb-2 border-2 rounded transition-shadow duration-150 ${
                        snapshot.isDraggingOver
