    ...
    <span className="mr-1">
      {displayTags.length > 0 &&
        displayTags.map((tag) => (
          <span
            key={tag.name}
            className="inline-flex items-center px-1 py-0.5 bg-purple-50 text-purple-600 text-xs rounded-full mr-1"
          >
            <span
              className="cursor-pointer hover:bg-purple-100 min-h-[20px] flex items-center"
            ...
            {onRemoveTag && tagExistsInTitle(tag.name) && (
              <button
                ...
                className="ml-1 text-purple-400 hover:text-purple-600 min-w-[20px] min-h-[20px] flex items-center justify-center text-xs"
              >
                &times;
              </button>
            )}
