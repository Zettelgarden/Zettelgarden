...
describe('parseFilterValue', () => {
  it('should parse equality (default operator)', () => {
    expect(parseFilterValue('active')).toEqual({
      operator: 'eq',
      value: 'active',
    });
    expect(parseFilterValue('pending')).toEqual({
      operator: 'eq',
      value: 'pending',
    });
  });
  ...
  it('should parse greater than or equal', () => {
    ...
    expect(parseFilterValue('gte:10')).toEqual({
      operator: 'gte',
      value: '10',
    });
  });
  ...
  it('should parse less than or equal', () => {
    ...
    expect(parseFilterValue('lte:100')).toEqual({
      operator: 'lte',
      value: '100',
    });
  });
  ...
  it('should parse not equals', () => {
    expect(parseFilterValue('ne:closed')).toEqual({
      operator: 'ne',
      value: 'closed',
    });
  });
  ...
  it('should handle values with colons in them', () => {
    expect(parseFilterValue('gte:2024-01-01')).toEqual({
      operator: 'gte',
      value: '2024-01-01',
    });
  });
...
describe('parseFiltersString', () => {
  ...
  it('should parse multiple filters', () => {
    expect(parseFiltersString('status=active,priority=high')).toEqual({
      status: 'active',
      priority: 'high',
    });
  });
  ...
  it('should parse filters with operators', () => {
    expect(parseFiltersString('rating=gte:4,status=active')).toEqual({
      rating: 'gte:4',
      status: 'active',
    });
  });
...
describe('applyFiltersToCard', () => {
  ...
  it('should apply date comparison operators to ISO date fields', () => {
    ...
    expect(
      applyFiltersToCard(cardWithDate, { due_date: 'gt:2026-01-01' }),
    ).toBe(true);
    expect(
      applyFiltersToCard(cardWithDate, { due_date: 'lt:2026-01-01' }),
    ).toBe(false);
    expect(
      applyFiltersToCard(cardWithDate, { due_date: 'lte:2026-01-15' }),
    ).toBe(true);
  });
  ...
  it('should handle cards without structured_data', () => {
    ...
    expect(applyFiltersToCard(cardWithoutData, { title: 'Simple Card' })).toBe(
      true,
    );
