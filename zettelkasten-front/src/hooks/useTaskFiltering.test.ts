...
describe('useTaskFiltering - sorting', () => {
  ...
  test('sorts by scheduled_date', () => {
    const tasks: Task[] = [
      ...
      createTask({
        id: 4,
        title: 'Task 4',
        scheduled_date: new Date('2026-01-05'),
      }),
    ];
    ...
    const dates = result.current.tasksToDisplay.map(
      (t) => t.scheduled_date?.toISOString() ?? null,
    );
  ...
  test('sorts by updated_at', () => {
    ...
    const updatedDates = result.current.tasksToDisplay.map((t) =>
